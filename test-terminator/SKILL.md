...
这是本 skill 的心脏。**没有走完这五步，不准输出"评审通过"。**
...
3. **禁止反向污染**：KPI 评级（尤其"失守 P0 一票否决判 F"）**绝不**能成为下调任一缺口等级、隐藏缺口、或把 `[TT-FAIL]` 翻成 `[TT-PASS]` 的理由。为了让自评好看而把一个真 P0 说成"测不到"或降级处理，等同触发**红线三**，直接判不合格。
...
- v1.2.0 — 校准防火墙 + 诚实失败分支：新增「判定顺序」铁律(覆盖率/P0~P3 定级/闸门按客观证据**先定稿**，战报与 KPI 自评只能**读取**、禁止反向下调等级或翻转闸门 → 堵住"为了不判 F 而把真 P0 降级"的自保式合理化)；`[TT-SPLIT]` 分块与「子 Agent 注入」补失败处置(失败块/子 agent 标未评审、场景计缺口、覆盖率不计未跑部分、禁止 [TT-PASS]、降级新增结论 [TT-PARTIAL]，父评审不得用自身分析顶替失败子 agent)；Phase 5 与输出模板同步 [TT-PARTIAL]
- v1.1.0 — 新增「战报结算 + KPI 评级」紧迫感输出：守住的防线 / 失守的缺口 / 攻防比、终结者 KPI 评级表（S~F，P0 失守一票否决）、测试人员待领赏反向施压；融入压力升级机制与输出格式模板，顶部宣言补「攻防战 = 失守 = KPI 掉级 = 淘汰」，子 Agent 注入协议同步加入战报结算（禁用「击杀/阵亡」杀戮词）
- v1.0.1 — P0 修复：Step 0 评审前检查（需求对齐 + 代码可评审性 + 超大代码分块）、增量/回归评审协议、用户拒绝修复升级机制、多 skill 冲突解决
- v1.0.0 — 初始版本，测试覆盖门控 + 三条红线 + 压力升级 + 抗合理化
