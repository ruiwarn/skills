...
Review staged diffs, decide whether to fix issues or proceed, and if proceeding generate a structured Chinese commit message, commit the staged changes, and push to remote.
...
- Run `git push` to the default remote/branch after a successful commit.
- If push fails (no upstream/permissions), report the error and ask how to proceed.
